"""Python translation of MATLAB Everycom_Per_IU.m."""
...
def main():
    ...
    for number in range(1, 11):
        ...
        Per_m_current = 0.08 * number
        iu_count_per_community = round(user_per_community * Per_m_current)
    ...
    plt.xlim([6, number * 6])
    plt.xticks(x_data)
    plt.xlabel("Percent of IU (%)", fontsize=18)
    plt.ylabel("QoE", fontsize=18)
    plt.legend(["Proposed", "LMPC", "GMPC", "HC", "RAC", "NCC", "NAV"], loc="best", fontsize=12)
    plt.grid(True)
    plt.savefig("Figure/Everycom_Per_IU.png", bbox_inches="tight")
