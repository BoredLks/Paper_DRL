"""Python translation of MATLAB SBS_IU_CacheSize.m."""
...
def main():
    ...
    D_eg_base = 30 * nf * v_chi_star
    D_iu_base = 10 * nf * v_chi_star
    ...
    for x in range(1, 11):
        D_eg_current = 5 * x * nf * v_chi_star
        for y in range(1, 11):
            D_iu_current = 2 * y * nf * v_chi_star
            ...
            print(f"SBS缓存 {x*5} Files, IU缓存 {y*2} Files, 平均QoE={qoe_grid[x-1, y-1]:.6f}")
    ...
    ax.set_xticklabels(5 * X_vals)
    ax.set_yticks(Y_vals)
    ax.set_yticklabels(2 * Y_vals)
    ax.set_xlabel("Cache size of SBS (Files)", fontsize=12)
    ax.set_ylabel("Cache size of IU (Files)", fontsize=12)
    ax.set_zlabel("QoE", fontsize=12)
    plt.tight_layout()
    plt.savefig("Figure/SBS_IU_CacheSize.png", bbox_inches="tight")
