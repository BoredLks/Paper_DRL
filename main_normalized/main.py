"""Python translation of MATLAB main.m (main_normalized)."""
...
if __package__ in (None, ""):
    ...
    from _algo_runner import build_args_kwargs, run_jobs_parallel  # type: ignore
    from cal_Movielens import DEFAULT_ML_PATH  # type: ignore
    from sim_utils import apply_global_seed  # type: ignore
else:
    from ._algo_runner import build_args_kwargs, run_jobs_parallel
...
def main():
    ...
    hit_rate_algorithm_number = np.zeros((7, 10))

    for number in range(1, 11):
        print(f"处理参数 {number}/10...")

        D_eg_current = 5 * number * nf * v_chi_star
        D_iu_current = 2 * number * nf * v_chi_star
        ...
        for algorithm_num in range(1, 8):
            ...
            jobs = [(algorithm_num, args_common, kwargs) for _ in range(run_num)]
            ...
            for res in results:
                if isinstance(res, tuple) and len(res) == 2:
                    ...
                    hit_vals.append(res[1])
                else:
                    qoe_vals.append(res)
            qoe_algorithm_number[algorithm_num - 1, number - 1] = float(np.mean(qoe_vals))
            if hit_vals:
                hit_rate_algorithm_number[algorithm_num - 1, number - 1] = float(np.mean(hit_vals))
            print(
    ...
    x_data = np.arange(1, number + 1) * 10
    ...
    plt.xlim([10, number * 10])
    plt.xticks(x_data)
    plt.xlabel("Cache size of System (Files)", fontsize=18)
    plt.ylabel("QoE", fontsize=18)
    plt.legend(["Proposed", "LMPC", "GMPC", "HC", "RAC", "NCC", "NAV"], loc="best", fontsize=12)
    plt.grid(True)
    plt.savefig("Figure/main.png", bbox_inches="tight")

    plt.figure(figsize=(8, 6))
    plt.plot(x_data, hit_rate_algorithm_number[0, :], "-o", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[6, :], "-^", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[5, :], "-v", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[1, :], "-*", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[3, :], "-s", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[4, :], "-d", linewidth=3)
    plt.plot(x_data, hit_rate_algorithm_number[2, :], "-x", linewidth=3)
    plt.xlim([10, number * 10])
    plt.xticks(x_data)
    plt.xlabel("Cache size of System (Files)", fontsize=18)
    plt.ylabel("Hit rate", fontsize=18)
    plt.legend(["Proposed", "NAV", "NCC", "LMPC", "HC", "GMPC", "RAC"], loc="best", fontsize=12)
    plt.grid(True)
    plt.savefig("Figure/Hit_rate.png", bbox_inches="tight")
